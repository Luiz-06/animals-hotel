      ...
      <div className="space-y-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Gerencie os tutores e seus pets</p>
          </div>
          <Button onClick={handleNewTutor} className="gap-2 shadow-lg"><Plus className="w-5 h-5" /> Novo Tutor</Button>
        </div>

        {tutores.length === 0 ? (
          <div className="text-center py-16 bg-muted/30 rounded-xl border-2 border-dashed">
            <p className="text-lg text-muted-foreground mb-4">Nenhum tutor encontrado.</p>
            <Button onClick={handleNewTutor} variant="outline" className="gap-2"><Plus className="w-5 h-5" /> Cadastrar Primeiro Tutor</Button>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {tutores.map((tutor) => (
              <TutorCard key={tutor.id} tutor={tutor} onEdit={handleEdit} onDelete={handleDelete} />
            ))}
          </div>
        )}
